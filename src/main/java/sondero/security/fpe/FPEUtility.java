...
public class FPEUtility {
    ...
    BigInteger Fk(int n, byte[] T, int i, BigInteger B, byte[] aesKey, StringBuilder log) {
        ...
        if (Bbytes.length > b) {
            throw new RuntimeException("The entered string is longer than " + b);
        }
        ...
        if (Y.length < d+4) {
            throw new RuntimeException("Sorry,Unsupported entry. We only support string 9 characters long for now.");
        }
